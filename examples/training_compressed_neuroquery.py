...
output_directory = "trained_compressed_text_to_brain_model"
...
dictionary = fetch_atlas_modl()["components_128"]
...
brain_maps, masker = coord_to_maps(
    coordinates, target_affine=(4, 4, 4), fwhm=None
)
...
######################################################################
# Get compressed representations of brain maps
D = masker.transform(dictionary).T
compressed = np.linalg.multi_dot(
    (np.linalg.pinv(D.T.dot(D)), D.T, brain_maps.values.T))
...
regressor.fit(tfidf, compressed)
print("Done")
